...
def get_current_provider():
    ...
    return os.environ.get('DB_PROVIDER', 'supabase')
...
class MongoDBProvider(DatabaseInterface):
    ...
    async def update_user(self, user_id: str, updates: Dict[str, Any]) -> bool:
        ...
        try:
            from bson import ObjectId
            result = self.db.users.update_one({"_id": ObjectId(user_id)}, {"$set": updates})
    ...
    async def get_user_projects(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            from bson import ObjectId
            projects = list(self.db.projects.find({"user_id": user_id}).sort("created_at", -1))
    ...
    async def update_project(self, project_id: str, updates: Dict[str, Any]) -> bool:
        ...
        try:
            from bson import ObjectId
            result = self.db.projects.update_one({"_id": ObjectId(project_id)}, {"$set": updates})
            return result.modified_count > 0
        except Exception as e:
            logger.error(f"Error updating project: {e}")
            return False
